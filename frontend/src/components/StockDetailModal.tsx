...
export const StockDetailModal: React.FC<StockDetailModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="modal-animate-in surface-card rounded-2xl max-w-3xl w-full p-4 sm:p-5 space-y-3.5 shadow-2xl border border-slate-700/60 my-auto max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
          <div className="flex items-center space-x-2.5 flex-wrap gap-y-1">
            ...
            {latest && (
              <span className={`text-sm sm:text-base font-mono font-bold ${latest.pct_chg >= 0 ? 'text-red-400' : 'text-emerald-400'}`}>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-xl flex items-center space-x-1 shadow transition-colors"
            >
        ...
        {dataHealth.status === 'mock' && (
          <div className="rounded-xl border border-amber-800/50 bg-amber-950/30 px-3 py-1.5 text-xs text-amber-200">
        ...
        {/* Chart Area */}
        {isLoading ? (
          <div className="h-72 sm:h-80 flex items-center justify-center text-slate-500 text-xs">
            行情与 K 线数据加载中...
          </div>
        ) : (
        ...
        {dataHealth.updatedAt && <p className="text-right text-[10px] text-slate-500">数据源：{dataHealth.source} · 更新于 {dataHealth.updatedAt} · <kbd className="font-mono text-slate-400">Esc</kbd> 退出</p>}
      </div>
    </div>
  );
};
